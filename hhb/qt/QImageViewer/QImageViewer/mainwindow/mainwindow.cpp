...
void MainWindow::openActionTriggered(void)
{
    int ret = imageViewer->openImageFile(tr("Select image:"),
                                         "/opt/qt5.5.1/apps/QDesktop/Resource/images",
...
{
    if(watched == toolBar || watched == menuBar){
        static QPoint point;
        switch (event->type()) {
        ...
        case QEvent::MouseButtonRelease:
        {
            ...
            if(offsety > 2){ // 判断如果这个值大于2，就出现关闭按钮
                ...
                pushButton->setStyleSheet("QPushButton{border-image:url(:/icons/images/close2.png);"
                                          "background-color:transparent}"
                                          "QPushButton:hover{border-image:url(:/icons/images/close2_hover.png);}"
                                          );
