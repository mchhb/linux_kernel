...
void MainWindow::mediaListInit()                                         //媒体列表初始化
{
    ...
    mediaList->setGeometry((float)550/800*this->geometry().width(),
                           ...
                           (float)385/480*this->geometry().height()
...
void MainWindow::pushButtonInit()
{
    ...
    openFilePushButton->setGeometry((float)700/800*toolWidgetPanel->geometry().width(), (float)23/75*toolWidgetPanel->geometry().height(),
                                    (float)20/800*toolWidgetPanel->geometry().width(), (float)20/800*toolWidgetPanel->geometry().width());//700,428,20,20
    mutePushButton->setGeometry((float)740/800*toolWidgetPanel->geometry().width(), (float)23/75*toolWidgetPanel->geometry().height(),
                                (float)20/800*toolWidgetPanel->geometry().width(), (float)20/800*toolWidgetPanel->geometry().width());//740,428,20,20
...
void MainWindow::sliderInit()
{
    ...
    mediaSlider->setGeometry((float)212/800*toolWidgetPanel->geometry().width(),
                             (float)36/75*toolWidgetPanel->geometry().height(),
                             (float)350/800*toolWidgetPanel->geometry().width(),
                             (float)25/75*toolWidgetPanel->geometry().height());//212,441,350,200设置位置与大小
    ...
    volumeslider->setGeometry((float)740/800*this->geometry().width(),
                              (float)302/480*this->geometry().height(),
                              (float)20/800*this->geometry().width(),
...
{
    delete ui;
    delete timerVolumeSlider;

    mediaPlayer->stop();
    mediaPlayList->clear();
    mediaObjectInfo.clear();//清空容器数组

    delete previousPushButton;
    delete nextPushButton;
    delete playOrPausePushButton;
    delete openFilePushButton;
    delete mutePushButton;

    delete mediaSlider;
    delete volumeslider;
    delete labelDuration;
    delete labelmediasTitle;
