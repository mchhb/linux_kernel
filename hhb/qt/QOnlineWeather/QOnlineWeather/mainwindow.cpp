...
void MainWindow::mainWindowInit()
{
    ...
    cityPushButton->setIcon(QIcon("Resource/icons/position.png"));
...
MainWindow::~MainWindow()
{
    ...
    delete currentTime;
    delete currentTimeLael;
    for(int i = 0; i < 5; i++){
        delete propertyAnimation[i];
        delete loopGraphicsDropShadowEffect[i];
        delete vlayoutArr[i];
        delete weekLabelArr[i];
        delete weatherTypeLabelArr[i];
        delete minMaxTempLabelArr[i];
        delete weatherIconLabelArr[i];
