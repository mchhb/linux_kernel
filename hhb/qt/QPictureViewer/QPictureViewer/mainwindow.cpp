...
#define  DEFAULT_WORK_DIR   "../QDesktop/Resource/images"
...
MainWindow::~MainWindow()
{
    ...
    delete pbPrevious;
    delete pbNext;
    delete pbRotatePositive90;
    delete pbRotateNegative90;
    delete pbListMode;
    delete pbPreviewMode;
    delete pbFullScreenMode;
    delete bottomMenuBar;
    
    imageDisplayList->clear();
    imageArr.clear();
    
    delete imageDisplayList;
    delete dynamicUpdateTimer;
    delete playTimer;
    delete hideMenuBarTimer;
    delete topTitle;
    delete lastCoordinate;
}
...
void MainWindow::buttonInit(void) {
    ...
    /* 确定控件坐标 */
    bottomMenuBar->setGeometry(0, this->geometry().height()-40, this->geometry().width(), 40);
...
void MainWindow::setListModeButtonGeometry(void) {
    pbPlay->setGeometry((float)320/800*this->geometry().width(), 5, 30, 30);
    pbPreviewMode->setGeometry((float)380/800*this->geometry().width(), 5, 30, 30);
    pbFullScreenMode->setGeometry((float)440/800*this->geometry().width(), 5, 30, 30);
}

void MainWindow::setPreviewModeButtonGeometry(void) {
    pbPrevious->setGeometry((float)310/800*this->geometry().width(), 5, 30, 30);
    pbPlay->setGeometry((float)250/800*this->geometry().width(), 5, 30, 30);
    pbNext->setGeometry((float)380/800*this->geometry().width(), 5, 30, 30);
    pbListMode->setGeometry((float)450/800*this->geometry().width(), 5, 30, 30);
    pbFullScreenMode->setGeometry((float)520/800*this->geometry().width(), 5, 30, 30);
}

void MainWindow::setFullScreenModeButtonGeometry(void) {
    pbPrevious->setGeometry((float)310/800*this->geometry().width(), 5, 30, 30);
    pbReduce->setGeometry((float)240/800*this->geometry().width(), 5, 30, 30);
    pbEnlarge->setGeometry((float)170/800*this->geometry().width(), 5, 30, 30);
    pbPlay->setGeometry((float)100/800*this->geometry().width(), 5, 30, 30);
    pbNext->setGeometry((float)380/800*this->geometry().width(), 5, 30, 30);
    pbRotateNegative90->setGeometry((float)450/800*this->geometry().width(), 5, 30, 30);
    pbRotatePositive90->setGeometry((float)500/800*this->geometry().width(), 5, 30, 30);
    pbListMode->setGeometry((float)590/800*this->geometry().width(), 5, 30, 30);
    pbPreviewMode->setGeometry((float)660/800*this->geometry().width(), 5, 30, 30);
}
...
void MainWindow::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if(mousePoint.y() < (float)150/480*this->geometry().height())     //防止用户滑动大于150的区域而触发关闭按钮出现
    {
        ...
        if(offset > 20)
        {
            ...
            pushButton->setStyleSheet("QPushButton{border-image:url(:/icon/Resource/close.png);"
                                      "background-color:transparent}"
                                      "QPushButton:hover{border-image:url(:/icon/Resource/close_hover.png);}"
                                      );
