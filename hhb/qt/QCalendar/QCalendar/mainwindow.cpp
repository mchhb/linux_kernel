    ...
    : QMainWindow(parent)
{
    ...
    year_month->setGeometry(0, (float)96/480*this->geometry().height(),
                            (float)160/800*this->geometry().width(),
    ...
    switch_year_month->setGeometry((float)40/800*this->geometry().width(),
                                   (float)24/480*this->geometry().height(),
    ...
    prev_month->setGeometry((float)480/800*this->geometry().width(),
                            (float)38/480*this->geometry().height(),
...
MainWindow::~MainWindow()
{
    ...
    delete week;
    delete year_month;
    delete switch_year_month;
    delete parent_widget;
    delete calendar_widget;
}
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if(watched == this){                                    //定义事件过滤器，指定在主窗口上检测
        ...
        case QEvent::MouseButtonRelease:
        {
            ...
            if(pt.y() > (float)150/480*this->geometry().height())// 限定触发区域
                return true;
