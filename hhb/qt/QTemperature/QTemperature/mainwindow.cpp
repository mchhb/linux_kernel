    ...
    : QMainWindow(parent)
{
    ...
    labelTitle->setText("板载温度传感器实时温度采集系统");
    ...
    labelTitle->setGeometry((float)10/800*this->geometry().width(), 0,
                            (float)700/800*this->geometry().width(), (float)40/480*this->geometry().height());
...
MainWindow::~MainWindow()
{
    ...
    delete process;
    delete labelTitle;
    delete labelCurrentTemp;
    delete labelTempUpdate;
    delete pbReset;
    delete pbStop;
}
...
{
    if(watched == this || watched == customPlot){
        static QPoint point;
        switch (event->type()) {
